...
export class KairosSocket {
  ...
  send(type: string, data?: Record<string, unknown>) {
    if (this.ws?.readyState !== WebSocket.OPEN) {
      console.warn(`[KairosSocket] send "${type}" dropped — not connected to ${this.url}`)
      return
    }
  ...
  sendBinary(data: ArrayBuffer) {
    if (this.ws?.readyState !== WebSocket.OPEN) {
      console.warn(`[KairosSocket] sendBinary dropped — not connected to ${this.url}`)
      return
    }
    this.ws.send(data)
  }
