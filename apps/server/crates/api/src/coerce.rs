...
///
/// Handles all 8 OSC addresses under the `/kairos/` prefix.
///
/// NOTE: The `/kairos/` prefix is kept for backward compatibility with
...
pub fn parse_osc(address: &str, args: &[OscType]) -> Result<RemoteCommand, CommandError> {
    match address {
        "/kairos/next" => Ok(RemoteCommand::Next),
        "/kairos/prev" => Ok(RemoteCommand::Prev),
        "/kairos/show" => Ok(RemoteCommand::Show),
        "/kairos/hide" => Ok(RemoteCommand::Hide),
        "/kairos/theme" => {
...
mod tests {
    ...
    #[test]
    fn parse_osc_next() {
        assert_eq!(parse_osc("/kairos/next", &[]).unwrap(), RemoteCommand::Next);
    }

    #[test]
    fn parse_osc_theme() {
        assert_eq!(
            parse_osc("/kairos/theme", &[OscType::String("Minimal".into())]).unwrap(),
    ...
    fn parse_osc_on_air_from_float() {
        assert_eq!(
            parse_osc("/kairos/on_air", &[OscType::Float(1.0)]).unwrap(),
    ...
    fn parse_osc_missing_arg_errors() {
        assert!(parse_osc("/kairos/opacity", &[]).is_err());
    }
}
