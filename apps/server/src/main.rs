...
use kairos_detection::semantic::index::VectorIndex;
use kairos_detection::{DetectionPipeline, QuotationMatcher, SemanticDetector};
...
async fn main() {
    ...
    tracing::info!("Kairos server listening on http://{addr}");
...
fn try_load_semantic(embeddings_path: &Path, ids_path: &Path) -> Option<SemanticDetector> {
    ...
    match kairos_detection::HnswVectorIndex::load(embeddings_path, ids_path, dim) {
        Ok(index) => {
            tracing::info!("Vector index loaded: {} vectors", index.len());
            Some(SemanticDetector::new(
                Box::new(kairos_detection::semantic::embedder::StubEmbedder::new(dim)),
                Box::new(index),
            ))
        }
        Err(e) => {
...
fn try_load_semantic_with_api(
    ...
    match kairos_detection::HnswVectorIndex::load(embeddings_path, ids_path, dimension) {
        Ok(index) => {
            tracing::info!("Vector index loaded with API embedder: {} vectors", index.len());
            let embedder = kairos_detection::ApiEmbedder::new(api_key, model, dimension);
            Some(SemanticDetector::new(
                Box::new(embedder),
                Box::new(index),
            ))
