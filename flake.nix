{
  description = "Kairos dev environment";
    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        buildScript = pkgs.writeShellScriptBin "build" ''
          ...
          echo "  ✓ apps/server/target/release/kairos-server"
        ...
        startScript = pkgs.writeShellScriptBin "start" ''
          ...
          API_BIN="$PROJECT_ROOT/apps/server/target/release/kairos-server"
          WEB_DIR="$PROJECT_ROOT/apps/web/dist"
          ...
          echo ""
          echo "  web → http://localhost:4000  (kairos.tensorkit.net)"
          echo "  api → http://localhost:4001  (kairos-api.tensorkit.net)"
          echo ""
        '';
      ...
      in {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            export KAIROS_DEV_DIR="$PWD/.dev"
            export PROJECT_ROOT="$PWD"
            export PORT=4001
            export HOST=0.0.0.0
            export DB_PATH="$PWD/apps/server/data/kairos.db"
            ...
            echo ""
            echo "  kairos dev shell"
            echo "  api → :4001 (kairos-api.tensorkit.net)"
            echo "  web → :4000 (kairos.tensorkit.net)"
            echo ""
