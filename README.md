...
<p align="center">
  ...
  <br />
  <a href="#" target="_blank"><strong>Try it live</strong></a> &middot; <a href="https://github.com/openbezal/rhema">Rhema Desktop</a> &middot; <a href="#quick-start">Quick Start</a>
</p>

---

Kairos is the cloud companion to [Rhema](https://github.com/openbezal/rhema), a desktop application for real-time Bible verse detection during live sermons. Kairos brings the same multi-strategy detection pipeline to the browser — no downloads, no installation, no setup beyond a Deepgram API key.

A preacher says _"nothing can separate us from God's love"_ and Kairos surfaces **Romans 8:38-39** in real-time, ready for your broadcast overlay.
...
Kairos removes that barrier. Open a URL, enter your Deepgram key, and start detecting verses. If it changes how you do live services — and we think it will — [Rhema Desktop](https://github.com/openbezal/rhema) is there when you're ready for the full broadcast production experience.

## Detection Pipeline

Kairos doesn't use a single method to find verses. It runs four strategies simultaneously and merges results with confidence weighting:
...
Visit [kairos.tensorkit.ai](https://kairos.tensorkit.ai), enter your [Deepgram API key](https://console.deepgram.com), and start transcribing.

### Self-Hosted

```bash
git clone https://github.com/tensorkithq/kairos.git
cd kairos
...
2. **Web** — set Root Directory to `apps/web`
    - Add env var `VITE_API_URL` = the server service's public URL (e.g. `https://kairos-server-production.up.railway.app`)
    - Railway auto-assigns `PORT`
...
| `DB_PATH`            | No       | ./data/kairos.db | Bible database path                                           |
...
packages
├── streams              — @kairos/streams RxJS orchestration library
└── overlay              — Broadcast overlay (standalone)
``` -->

## Design Decisions

We made deliberate trade-offs to keep Kairos simple and portable:
...
## Kairos vs Rhema Desktop

Kairos is not a replacement for Rhema. It's the evaluation ramp.

| Capability       | Kairos (Cloud)       | Rhema (Desktop)                     |
| ---------------- | -------------------- | ----------------------------------- |
...
## Stream Orchestration (`@kairos/streams`)

Kairos uses a shared RxJS library at [`packages/streams`](packages/streams/) to manage all real-time data flow. The library is framework-agnostic — it produces observables that the React app subscribes to and pushes into Zustand stores.

```
Events (WebSocket, user input) → @kairos/streams (compose, cancel, debounce) → Zustand (state) → React (UI)
```
...
Kairos's detection pipeline reuses Rhema's Rust crates server-side, but the client-side orchestration differs:

| Capability              | Kairos                           | Rhema Desktop                      |
| ----------------------- | -------------------------------- | ---------------------------------- |
| Stream orchestration    | RxJS (`@kairos/streams`)         | Tauri event system + Rust channels |
...
The WebSocket boundary means Kairos pays latency that Rhema avoids with in-process communication. The RxJS layer mitigates this by keeping the UI responsive (non-blocking streams, automatic cancellation) while the server handles the heavy detection work.
