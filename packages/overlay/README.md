...
Renders Bible verses on a transparent canvas at configurable resolution, controlled via WebSocket from the main Kairos dashboard.
...
```
Both `apps/web/` and `packages/overlay/` import from `@kairos/shared`.

### Phase 3: Standalone overlay app (this package)
Move overlay out of `apps/web/` into this package:
```
packages/overlay/
  package.json      → @kairos/overlay (depends on @kairos/shared)
