...
RxJS stream orchestration library for Kairos. Framework-agnostic — produces observables that any UI layer can subscribe to.

## Install

```bash
pnpm add @kairos/streams rxjs
```
...
```ts
import { fromSocketEvent } from "@kairos/streams"
...
```ts
import { createTranscriptionStream, createSearchStream } from "@kairos/streams"
...
```ts
import { StreamOrchestrator } from "@kairos/streams"
...
```ts
import { fallbackChain } from "@kairos/streams"
...
```
Events (WebSocket, user input) → @kairos/streams (compose, cancel, debounce) → State (Zustand)
```
