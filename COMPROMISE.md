...
Kairos runs Rhema's detection pipeline in a browser. That constraint forced trade-offs that don't exist in the desktop app. This document records what we gave up, what we gained, and why.
...
**The compromise:** We added RxJS (`@kairos/streams` package) as the orchestration layer between events and state. `switchMap` replaces request ID counters, `debounceTime` replaces `setTimeout` refs, `merge` replaces multi-listener cleanup arrays, `interval` replaces `setInterval`.
...
- Framework-agnostic core. The `@kairos/streams` package has zero React dependencies. It produces observables that any UI layer can subscribe to.
...
**What we gained:**
- Shared Rust crates. The detection pipeline is the same code running in both Kairos's server and Rhema's desktop app. No JavaScript rewrite, no WASM porting, no accuracy divergence.
